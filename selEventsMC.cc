...
void selEventsMC(){
  ...
  TH1F * h_stuff = new TH1F("stuff", "stuff", 11,0,11);  
  ...
  for (Long64_t i =0 ;true; ++i){
    ...
    if (tautau_baseline) h_stuff->Fill(0);
    if (mutau_baseline) h_stuff->Fill(1);
    if (etau_baseline) h_stuff->Fill(2);
    ...
    if (btagM) h_stuff->Fill(3);
    if (btagMM) h_stuff->Fill(4);
    if (btagLL) h_stuff->Fill(5);
    // mass
    ...
    if (ellypsMassCut) h_stuff->Fill(6);
    if (boostMassCut) h_stuff->Fill(7);
    ...
    if (tautau_SR) h_stuff->Fill(8);
    if (mutau_SR) h_stuff->Fill(9);
    if (etau_SR) h_stuff->Fill(10);
