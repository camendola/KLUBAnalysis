...
channelsName = ["bb #mu#tau_{h} channel","bb #tau_{h}#tau_{h} channel","bb #mu#tau_{h} + bb e#tau_{h} + bb #tau_{h}#tau_{h}"]
...
for c in range(len(tooLopOn)) :
	...
	for m in masses :
		...
		fileLocation = inputDir+"/cards_"+channels[c]+"_"+folder+"/"+app+catstring+"_"+str(m)+"/higgsCombine"+app+"_"+str(m)+"_forLim_noTH.AsymptoticLimits.mH"+str(m)+".root"
		if plotByCategory :
		    fileLocation = inputDir+"/cards_Combined_"+folder+"/"+app+catstring+"_"+str(m)+"/higgsCombine"+app+"_"+str(m)+"_forLim_noTH.AsymptoticLimits.mH"+str(m)+".root"
...
for ic in range(len(tooLopOn)):
	...
	if benchmark>-1:
		for ext in outFormats : cNice[ic].SaveAs("plots/2017stat/limit"+app2+"_"+tooLopOn[ic]+outString+ext)
		print "SAVED IN " , "plots/2017stat/limit"+app2+"_"+tooLopOn[ic]+outString+ext
