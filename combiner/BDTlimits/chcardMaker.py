...
def  writeCard(input,theLambda,select,variable,mass,region=-1) :
    ...
    if   "baseline" in select : theCat = "0"
    ...
    for bkg in input.background:
        ...
        #data_obs_baseline_SR_BDToutSM_kl_1
    ...
    if region < 0 :
        ...
        syst = systReader("../../config/systematics_VBF2017.cfg",[lambdaName],backgrounds,file)
        syst.writeOutput(False)
        syst.verbose(True)
        if(opt.channel == "TauTau" ): 
            syst.addSystFile("../../config/systematics_tautau_VBF2017.cfg")
            #if(opt.isResonant):
        ...
        cmb1.cp().signals().ExtractShapes(
            opt.filename,
            #"$PROCESS$MASS_$BIN_{1}_{0}".format(variables[0],regionSuffix[region+1]),                  # !!FIXME!! not sure why this does not work!
            #"$PROCESS$MASS_$BIN_{1}_{0}_$SYSTEMATIC".format(variables[0],regionSuffix[region+1]))      # !!FIXME!! not sure why this does not work!
            "$PROCESS_$BIN_{1}_{0}".format(variables[0],regionSuffix[region+1]),
            "$PROCESS_$BIN_{1}_{0}_$SYSTEMATIC".format(variables[0],regionSuffix[region+1]))
